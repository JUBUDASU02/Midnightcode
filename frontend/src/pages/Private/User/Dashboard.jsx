...
/* ─────────────────────────────────────────────
   PROFILE DROPDOWN  (definido aquí mismo,
   no necesita archivo separado)
───────────────────────────────────────────── */
function ProfileDropdown({ user, onProfile, onLogout, onClose }) {
  ...
  useEffect(() => {
    ...
    // pequeño delay para que el clic que lo abre no lo cierre inmediatamente
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "12px 14px 12px", borderBottom: "1px solid rgba(255,255,255,0.05)", marginBottom: "4px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <img
            src={user.avatar}
            alt={user.name}
            style={{ width: "38px", height: "38px", borderRadius: "50%", objectFit: "cover", border: "1px solid rgba(138,43,226,0.45)", flexShrink: 0 }}
          />
          <div style={{ minWidth: 0 }}>
            <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "13px", color: "#fff", margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {user.name}
            </p>
            <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "10px", color: "#c084fc", margin: 0, letterSpacing: "0.5px" }}>
...
export default function UserDashboard() {
  ...
  /* Ir al perfil desde el dropdown o sidebar */
  ...
  if (activePage === "reserve") {
    return <ReservationPage event={selectedEvent} onBack={() => { setActivePage("dashboard"); setActiveNav("events"); }} />;
  }
  if (activePage === "songs") {
    return <SongRequestPage onBack={() => { setActivePage("dashboard"); setActiveNav("dashboard"); }} />;
  }
  if (activePage === "profile") {
    return <ProfilePage user={CURRENT_USER} onBack={() => { setActivePage("dashboard"); setActiveNav("dashboard"); }} />;
  }
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#080810", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Syne',sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <h2 style={{ fontWeight: 800, fontSize: "28px", color: "#fff", margin: "0 0 8px" }}>Sesión cerrada</h2>
          <p style={{ color: "#4a4a6a", fontFamily: "'Space Mono',monospace", fontSize: "12px", marginBottom: "28px" }}>
            Hasta pronto, {CURRENT_USER.name.split(" ")[0]}
          </p>
          <button
            onClick={() => setActivePage("dashboard")}
            style={{ padding: "13px 30px", background: "linear-gradient(135deg,#8a2be2,#d400ff)", border: "none", borderRadius: "12px", color: "#fff", fontSize: "14px", fontWeight: 700, fontFamily: "'Syne',sans-serif", cursor: "pointer" }}
          >
            Volver a ingresar
          </button>
        </div>
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#080810", width: "100%", overflow: "hidden", fontFamily: "'Syne',sans-serif" }}>
      ...
      <aside style={{ width: "72px", background: "#0d0d1a", borderRight: "1px solid rgba(138,43,226,0.15)", display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 0", gap: "8px", flexShrink: 0 }}>
        ...
        {/* Avatar en sidebar → va directo al perfil */}
        <div
        ...
        >
          <img
            ...
            alt="perfil"
            style={{ width: "36px", height: "36px", borderRadius: "50%", objectFit: "cover", border: "1px solid rgba(138,43,226,0.4)", display: "block" }}
          />
        </div>
      </aside>
            ...
            <div style={{ position: "relative" }}>
              ...
              <div
                ...
                style={{
                  ...
                  background: "#0d0d1a",
                  borderRadius: "40px",
                  border: `1px solid ${dropOpen ? "rgba(138,43,226,0.55)" : "rgba(255,255,255,0.08)"}`,
                  cursor: "pointer",
                }}
              >
                <img
                  src={CURRENT_USER.avatar}
                  alt="avatar"
                  style={{ width: "32px", height: "32px", borderRadius: "50%", objectFit: "cover", border: "1px solid rgba(138,43,226,0.35)" }}
                />
                <div>
                  <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "13px", color: "#fff", margin: "0 0 1px" }}>
                    {CURRENT_USER.name.split(" ")[0]}
                  </p>
                  <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "9px", color: "#c084fc", margin: 0, letterSpacing: "0.5px" }}>
                    VIP
                  </p>
                </div>
                <span style={{ color: "#4a4a6a", fontSize: "9px", marginLeft: "2px" }}>
                  {dropOpen ? "▲" : "▼"}
                </span>
              </div>

              {/* Dropdown — solo se renderiza cuando dropOpen === true */}
              {dropOpen && (
                <ProfileDropdown
                  user={CURRENT_USER}

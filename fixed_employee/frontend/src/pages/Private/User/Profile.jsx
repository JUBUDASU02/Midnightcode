...
export default function ProfilePage({ user: propUser, onBack }) {
  // ✅ BUG-05 FIX: usar useAuth() para obtener updateUser
  ...
  // ✅ BUG-05 FIX: save() llama a updateUser() para persistir cambios en el contexto
  const save = () => {
    updateUser(form);
    setSaved(true);
      ...
      <div style={{ display:"grid", gridTemplateColumns:"240px 1fr", gap:"28px", alignItems:"start" }}>
        ...
        <div style={{ background:"#0d0d1a", border:"1px solid rgba(138,43,226,0.12)", borderRadius:"20px", padding:"28px", textAlign:"center" }}>
          <div style={{ position:"relative", width:"fit-content", margin:"0 auto 20px" }}>
            <img src={user?.avatar||"https://i.pravatar.cc/200"} style={{ width:"96px", height:"96px", borderRadius:"50%", border:"2px solid rgba(138,43,226,0.5)", objectFit:"cover", display:"block" }} />
            <label className="upload-lbl" style={{ position:"absolute", bottom:"-4px", right:"-4px", background:"#8a2be2", borderRadius:"50%", width:"28px", height:"28px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"2px solid #080810", fontSize:"13px", color:"#fff" }}>
              ✎<input type="file" style={{ display:"none" }} />
            </label>
          </div>

          {/* ✅ BUG-05 FIX: mostrar nombre actualizado del form */}
          ...
          <div style={{ marginTop:"24px", paddingTop:"20px", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ background:"rgba(0,245,160,0.08)", borderRadius:"10px", padding:"10px 12px", border:"1px solid rgba(0,245,160,0.15)" }}>
              ...
              <p style={{ color:"#fff", fontSize:"13px", fontWeight:600, margin:0 }}>{user?.memberSince||"2025"}</p>
            </div>
          </div>
        </div>
